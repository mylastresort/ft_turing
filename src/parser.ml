...
let mem_to_str obj name =
  obj |> member name |> to_string

let mem_to_assoc obj name =
...
let extract_state_trans state transition_list states alphabet actions =
  try
    ...
    transitions |> List.map (fun obj ->
      {
        read = validate_state_trans "read" (extract obj) state alphabet;
        to_state = validate_state_trans "to_state" (extract obj) state states;
...
let parse jsonfile =
  ...
  let initial = extract_field "initial" (mem_to_str json) in
  {
    ...
    finals = if List.for_all (
        fun e1 -> List.exists (fun e2 -> e1 = e2) states) finals then finals
      else
        raise (
          Json_malformed_value ("Bad json input: 'finals' has unknown state")
        );
    transitions = List.map (fun (state, transition_list) ->
      (validate_state state states, extract_state_trans state transition_list
